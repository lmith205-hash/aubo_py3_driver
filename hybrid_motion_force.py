...
# 1. 速度规划参数 (直线和圆弧通用)
TRAJ_MAX_VEL = 0.02     # XY平面巡航线速度: 5cm/s
TRAJ_ACC     = 0.02     # XY平面线加速度: 2cm/s^2

# 2. 直线参数
LINE_LENGTH  = 0.15     # 长度: 20cm

# 3. 圆弧参数 (用于 circle_center 模式)
FIXED_CENTER_XY = [0.7, 0.00] # 圆心绝对坐标 [x, y]
CIRCLE_ANGLE = 360.0   # 旋转总度数
...
# [新增] 低通滤波器类 (来自 constant_force_control.py)
class LowPassFilter:
        ...
        try:
            while not rospy.is_shutdown():
                ...
                if self.state == 0: # 寻找接触
                    ...
                    if is_force_stable or is_bottomed_out:
                        ...
                        if is_bottomed_out or (time.time() - self.contact_start_time > 1.0):
                            ...
                            if MOTION_TYPE == 'circle_center':
                                ...
                                print("    - 自动计算半径 R = {:.4f} m".format(self.circle_radius))
                                print("    - 起始角度 Theta = {:.2f} deg".format(math.degrees(self.circle_start_angle)))
                    ...
                    traj_pos, finished = self.get_trajectory_pos(t_motion)
                    
                    if finished:
                        print("\n>>> [事件] 轨迹运行完毕。")
                        self.state = 2 

                elif self.state == 2: # 保持原位，只做Z轴力控
                    pass
                
                # 3. 融合位置指令: 
                # Target = (XY Trajectory) + (Z Force Correction)
                target_pos_vec = traj_pos
                # 注意: Z轴基准是 initial_pos[2]，加上力控产生的 diff
